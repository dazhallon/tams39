...
x2 = [278 277 308 290 273 284 267 281 287 271 302 254 297 281 284 ...
 ...
 loglambda = (nm-1)/2 * log(det(Sm)) + (nf-1)/2*log(det(Sf))...
   - (n-2)/2*log(det(S));
 loglambda = - loglambda;
 p = size(Xf,2);
 g = 2;
...
u = ((nm-1)^-1 + (nf-1)^-1 - (n-2)^-1)*...
 ...
 disp('ahat')
 printMatrix(a)
 c = (n-2)*2/(n+1-p) * finv(1-alpha, p,n+1-p);
 intlength = c*sqrt(a'*(nf^-1 + nm^-1)*S*a);
 lhs = a'*y - intlength;
 rhs = a'*y + intlength;
 disp('ahat interval')
 printMatrix([lhs, rhs])
...
title('');
savefig(fig, 'report/images/ex2fig', 'epsc')
